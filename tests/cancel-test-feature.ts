...
describe("Cancel test Auction", async () => {
    ...
    it("should list a cnft and cancel listing", async () => {
        // create merkleTree
        ...
        const sellTx = await auctionHouseSdk.sell(SaleType.Auction, true, new anchor.web3.PublicKey(asset_id), {
        ...
        const cancelTx = await auctionHouseSdk.cancelTx(new anchor.web3.PublicKey(asset_id), {
            priceInLamports: BUY_PRICE * LAMPORTS_PER_SOL,
            merkleTree: landMerkleTree,
            paymentAccount: tokenAccount,
            wallet: owner,
