...
import getTestSetupV2 from "../../tests/utils/getTestSetupV2";
...
let metadataData: Maybe<DataV2>;
let programmableNftMetadata: Maybe<Metadata>;
...
describe("buy Programmable NFT tests (placing bids/offers)", () => {
  beforeAll(async () => {
    const setupResult = await getTestSetupV2({
      auctionHouseConfig: {
      ...
      connection,
      programCreatorWallet: WALLET_CREATOR,
  ...
  it.each([SaleType.InstantSale, SaleType.Offer])(
